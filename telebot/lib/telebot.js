...
class TeleBot {
  constructor(cfg) {
    ...
    this.buildInPlugins =
      cfg.buildInPlugins !== undefined
        ? cfg.buildInPlugins || []
        : BUILDIN_PLUGINS;
    this.buildInPluginsFolder =
      cfg.buildInPluginsFolder || BUILDIN_PLUGINS_FOLDER;
    ...
    this.allowedUpdates =
      typeof cfg.allowedUpdates === "string" ||
      Array.isArray(cfg.allowedUpdates)
        ? cfg.allowedUpdates
        : [];
    this.maxConnections =
      this.webhook && Number.isInteger(this.webhook.maxConnections)
        ? this.webhook.maxConnections
        : 40;
            ...
            if (eventType instanceof RegExp) {
              ...
              if (match) {
                ...
                promise = promise.then(() =>
                  bot.event(eventType, message, props)
                );
              }
            }
          }
          ...
          const SHORTCUTS = {
            ...
            place(position, title, address, props = {}) {
              ...
              return this.bot.sendVenue(
                ...
                title,
                address,
                this.propertyProcessor(msg, props)
              );
            },
          };
    ...
    // Load user plugins
    this.usePlugins.map((userPluginName) =>
      this.plug(require(`${this.pluginFolder}${userPluginName}`))
    );
  }
  ...
  static addMethods(methods) {
    for (let id in methods) {
      ...
      this.prototype[id] = this.prototype[name] = function () {
        ...
        return this.request(`/${id}`, form).then(
          method.then || ((re) => re && re.result)
        );
      };
    }
  }
  ...
  plug(module) {
    ...
    if (id) {
      ...
      const isConfigObject =
        Object.prototype.toString.call(defaultConfig) === "[object Object]";
    ...
    this.loopFn = setInterval(() => {
      ...
      this.getUpdates()
        ...
        .catch(() => {
          ...
          console.log(
            `[bot.info.update] reconnecting in ${seconds} seconds...`
          );
  ...
  receiveUpdates(updateList) {
    ...
    // We have updates
    return this.event("update", updateList)
      .then((eventProps) => {
